...
public class UserMemberController {
	...
	// 로그인
	// @RequestMapping(value="/member/loginAction.do", method=
	...
	//
	// MemberDTO vo=
	// sqlSession.getMapper(UserMemberImpl.class).login(req.getParameter("mem_id"),
	...
	public void loginaction(HttpServletRequest req, HttpServletResponse resp, HttpSession session,
			MemberDTO memberDTO, Model model) throws Exception {
		memberDTO = sqlSession.getMapper(UserMemberImpl.class).login(req.getParameter("mem_id"),
				req.getParameter("mem_pw"));
		if (memberDTO == null) {
	...
	public String findIdResult(HttpServletRequest req, HttpServletResponse resp, HttpSession session,
	...
	public String findPwResult(@RequestParam("mem_email") String mem_email, @RequestParam("mem_id") String mem_id,
			HttpServletRequest req, HttpServletResponse response, MemberDTO memberDTO) throws Exception {
	...
	// 회원가입 결과
	@RequestMapping(value = "/member/joinResult.do", method = RequestMethod.POST)
	...
	public String delete(HttpSession session, HttpServletRequest req) {
		...
		int applyRow = sqlSession.getMapper(UserMemberImpl.class).delete(req.getParameter("mem_id"));
	...
	public String kakao(MemberDTO dto, HttpServletRequest req, HttpSession session) throws Exception {
		...
		if (sqlSession.getMapper(UserMemberImpl.class).check_id(kakaoid) == 0
				&& sqlSession.getMapper(UserMemberImpl.class).check_email(kakaoemail) == 0) {
