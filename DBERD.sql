...
(
	...
	-- 점주이름
	store_owner varchar2(50) DEFAULT '김점주',
	-- 전화번호
	store_phone varchar2(30) DEFAULT '02-111-1111',
	-- 아이템
	store_item varchar2(200) NOT NULL,
	-- 시군구
...
COMMENT ON COLUMN event_item.event_itemts_num IS '항목일련번호';
COMMENT ON COLUMN event_item.event_idx IS '이벤트일련번호';
COMMENT ON COLUMN event_item.event_items_name IS '항목명';
COMMENT ON COLUMN event_item.event_items_prbbl IS '확률';
COMMENT ON COLUMN event_item.event_item_ratio IS '비율';
COMMENT ON COLUMN event_item.event_items_result IS '항목결과';
...
COMMENT ON COLUMN member.mem_regidate IS '가입일';
COMMENT ON COLUMN member.mem_point IS '포인트';
COMMENT ON COLUMN member.mem_sub IS '보유구독권여부 혹은 일련번호';
